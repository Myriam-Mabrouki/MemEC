...
int main() {
  int fd0, fd[TOTAL_EVENTS-1];  // fd0 will be the group leader file descriptor
  ...
  fd0 = perf_event_open(&pe[0], 0, -1, -1, 0);
  ioctl(fd0, PERF_EVENT_IOC_ID, &id[0]);
  // Let's create the rest of the events while using fd0 as the group leader
  for(int i = 1; i < TOTAL_EVENTS; i++){
    fd[i-1] = perf_event_open(&pe[i], 0, -1, fd0, 0);
    ioctl(fd[i-1], PERF_EVENT_IOC_ID, &id[i]);
  }
